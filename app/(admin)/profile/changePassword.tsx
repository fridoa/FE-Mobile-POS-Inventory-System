  ...
  const { control, onSubmit, isLoading } = useChangePasswordHook(
    () =>
      setAlert({
        visible: true,
        title: "Berhasil",
        message: "Kata sandi diperbarui. Silakan login kembali.",
        type: "success",
        onConfirm: async () => {
          await logoutAction();
          router.replace("/(auth)");
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"}>
        ...
        <ScrollView contentContainerStyle={{ flexGrow: 1, paddingTop: 20 }} className="p-6">
          <View className="mt-8 mb-8">
            <Text className="text-3xl font-bold text-emerald-800">Keamanan Akun</Text>
            <Text className="mt-2 text-gray-500">Perbarui kata sandi Anda secara berkala untuk menjaga keamanan data toko.</Text>
          </View>
