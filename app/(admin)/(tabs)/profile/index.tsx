...
export default function AdminProfileScreen() {
  ...
  return (
    <View className="flex-1 bg-white">
      {/* 1. Gelombang Atas (Sama dengan Kasir) */}
      ...
      {/* 2. Gelombang Kanan Bawah (Sama dengan Kasir) */}
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }} bounces={false}>
        ...
        <View style={{ paddingTop: insets.top + 25 }} className="items-center px-6">
          ...
          <View className="mt-4 items-center">
            <Text className="text-2xl font-bold text-gray-900 capitalize">{user?.name || "Admin Owner"}</Text>
            <Text className="text-gray-500 font-medium mt-1">@{user?.username || "adminintan"}</Text>
          </View>
        </View>

        {/* Action Buttons Row - Perubahan dari List ke Row Ikon */}
        <View className="flex-row justify-center mt-8 space-x-10 gap-x-8">
            <TouchableOpacity onPress={() => router.push("/(admin)/profile/editProfil")} className="items-center space-y-2">
            ...
            <TouchableOpacity onPress={() => router.push("/(admin)/profile/changePassword")} className="items-center space-y-2">
            ...
            <View className="bg-white/80 p-5 rounded-2xl border border-gray-100 shadow-sm space-y-4">
                ...
                <View className="flex-row justify-between items-center">
                    ...
                    <View className="bg-blue-100 px-3 py-1 rounded-full">
                        <Text className="text-[10px] font-bold text-blue-700 uppercase">ADMINISTRATOR</Text>
                    </View>
                </View>
            </View>
        </View>
