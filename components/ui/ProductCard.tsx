...
export default function ProductCard({ item }: ProductCardProps) {
  ...
  const discountedPrice = hasDiscount ? item.price - (item.price * item.discount!) / 100 : item.price;
      ...
      <View className="flex-1 ml-4 justify-between h-[72px]">
        ...
        <View className="flex-row items-center mt-1">
          <View className="flex-1">
            ...
            {hasDiscount && <Text className="text-[10px] text-gray-400 line-through">Rp {item.price.toLocaleString("id-ID")}</Text>}
          </View>
        </View>
      </View>
    </View>
